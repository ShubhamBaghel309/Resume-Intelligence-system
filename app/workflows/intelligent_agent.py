...
2. Extract that candidate's name and add it to the "entities" → "names" field
3. Examples:
...
- **ACHIEVEMENTS, CERTIFICATIONS & OTHER INFORMATION** section contains:
  ...
  • Coding platform profiles (Codeforces, LeetCode ratings)
  • Publications, patents
  • Volunteer work, extracurriculars
  • Languages spoken, hobbies, interests
  
- **Projects** section contains:
  • Technical projects built by the candidate
  • Descriptions, technologies used, role
  
- **Work Experience** section contains:
  • Job roles, companies, responsibilities
...
def sql_filter_node(state: AgentState) -> AgentState:
    ...
    # ✅ NEW: Candidate IDs filter (for follow-up queries) - CHECK THIS FIRST!
    ...
    if not filters.get("candidate_ids"):
        ...
        if entities.get("names"):
            ...
            # For "Shubham Baghel" → match both "Shubham" AND "Baghel"
...
def vector_search_node(state: AgentState) -> AgentState:
    ...
    if state["search_strategy"] == "sql_only" and state["candidate_ids"]:
        print("\n🔍 SKIPPING VECTOR SEARCH (sql_only strategy)")
...
def generate_answer_node(state: AgentState) -> AgentState:
    ...
    # ✅ CRITICAL FIX: Limit to top 5 candidates to prevent hallucinations
